...
// Format movements date depending on locale settings and passed time
...
// Display currency depending on locale settings
...
// Displaying movements from accounts into containerMovements
// Can be sorted using second parameter
...
// Calculation and displaying balance of account depending on movements
const calcDisplayBalance = acc => {
  acc.balance = acc.movements.reduce((acc, mov) => acc + mov.value, 0);
  labelBalance.textContent = formatCur(acc.balance, acc.locale, acc.currency);
};

// Calculation and displaying summary of account depending on movements
...
// Creates usernames for accounts by taking first name
...
// Updating UI when need to change info on site
const updateUi = acc => {
  // Display movements
...
// Creates logout timer. 5 min by defaults
const startLogOutTimer = () => {
  // Format time in more appreciate format
  const tick = () => {
...
// Sorting movements from bigger to smaller
